...
function makeWord(lettersObject) {
  return Object.entries(lettersObject)
    .map(([letter, positions]) => positions.map((pos) => [pos, letter]))
    .flat()
    .sort((a, b) => a[0] - b[0])
    .map(([, letter]) => letter)
    .join('');
...
const NO_DUPLICATES = [ORDER.element, ORDER.id, ORDER.pseudo_element];

const cssSelectorBuilder = {
  result: '',

  element(value) {
    return this.addSelector(value, ORDER.element);
  },

  id(value) {
    return this.addSelector(`#${value}`, ORDER.id);
  },

  class(value) {
    return this.addSelector(`.${value}`, ORDER.class);
  },

  attr(value) {
    return this.addSelector(`[${value}]`, ORDER.attribute);
  },

  pseudoClass(value) {
    return this.addSelector(`:${value}`, ORDER.pseudo_class);
  },

  pseudoElement(value) {
    return this.addSelector(`::${value}`, ORDER.pseudo_element);
  },
  ...
  addSelector(value, order) {
    ...
    if (this.order === order && NO_DUPLICATES.includes(order)) {
      throw new Error(
